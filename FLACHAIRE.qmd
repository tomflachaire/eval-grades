...
This bar chart shows minimal variation by sex.
...
```{r}
# We can compute average grades per student for the two courses
...
This scatter plot illustrates a slight positive correlation between average grades in Electrical Engineering and Cryptography.
...
This scatter plot shows a negative correlation between grades in Airship Piloting and Navigation and Etiquette and Social Graces.
...
Groups 9 and 12 have the lowest and least consistent final grades compared to the other groups.
...
This box plot shows that final grades vary by group and sex, with some groups displaying significant differences in median grades and variability between males and females.

### Question 26

```{r}
# we can compute final grade and pass/fail
pass_df <- avg_per_course %>%
  ...
  summarise(
    ...
    module1 = mean(avg_course[module == 1]),
    module2 = mean(avg_course[module == 2]),
    .groups = "drop"
  ) %>%
  mutate(
    pass = (min_course >= 5) & (module1 >= 10) & (module2 >= 10)
  ) %>%
...
# Print as a sentence
cat("There are", n_students, "students who do not pass but have a final grade >= 10.")
```
...
# bar plot
ggplot(pass_rate, aes(x = factor(group), y = pass_rate)) +
  geom_col() +
  labs(x = "Group", y = "Pass Rate (%)", title = "Pass Rate by Group") +
  theme_minimal()
```

We can see graphically that group 9 and group 12 have a lot more difficulties than others, as we saw before.

### Question 29

```{r}
# we count number of students failing a course (avg < 5)
fail_course <- avg_per_course %>%
  filter(avg_course < 5) %>%
  group_by(course_id) %>%
  summarise(n_fail = n(), .groups = "drop") %>%
  left_join(courses %>% select(course_id, course), by = "course_id") %>%
  arrange(desc(n_fail))

# so the most responsible course
fail_course %>% slice_head(n = 1)
```
Alchemy and Chemical Engineering is the course that is the most responsible of student failure owing to grades below than 5.
...
# Age + success rate
pass_df %>%
  left_join(students %>% mutate(age = round(time_length(today() - birth_date, "year"))) %>% select(id, age), by = "id") %>%
  group_by(age) %>%
  summarise(success_rate = mean(pass) * 100, .groups = "drop") %>%
  ggplot(aes(x = age, y = success_rate)) +
  geom_line() +
  geom_point() +
  labs(x = "Age", y = "Success rate (%)", title = "Success rate by Age") +
  theme_minimal()
```

The success rate fluctuates significantly with age.
